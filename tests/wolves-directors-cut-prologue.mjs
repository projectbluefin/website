...
try {
  ...
  for (const record of cut.artwork) {
    ...
    assert(`${record.id} carries the legibility scrim instead`, stage.hasScrim, stage)
    assert(
  ...
  if (held) {
    ...
    assert('a cue with slack between its hold and its window exists', false)
  }
  ...
  const music = promoted.find(player => player.videoId === 'EB3IokHelRk')
