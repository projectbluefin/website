...
```bash
npm install --include=dev         # installs dev dependencies
...
- **Run `node tests/navbar-visual.mjs`** to validate navbar against docs.projectbluefin.io (38 Playwright assertions). Requires the dev server running at http://localhost:5173 and `playwright` installed.
- **Public asset fetches** — MUST use `import.meta.env.BASE_URL` prefix, not absolute `/` paths:
  ```ts
  fetch(`${import.meta.env.BASE_URL}knuckle-versions.json`) // correct
  fetch('/knuckle-versions.json') // wrong: breaks on subpath deploy
  ```
- **Build warnings:** Vite 8 + TailwindCSS 4 may print harmless `[lightningcss minify] Unknown at rule` warnings for `@theme` / `@tailwind`. The build output is correct.
