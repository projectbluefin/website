...
1. `wolves-prologue` — a black-screen, text-only cold open (no video id) set to a background-only YouTube audio embed (the Gayane Ballet Suite Adagio / "2001: A Space Odyssey" cue). Per-cue static or day/night-crossfade background artwork is supported for two specific beats (the Collapse image, a single KubeCon hero photo); every other beat stays plain black text. **Cut from 326s (the song's full 5:26) down to a 45s excerpt per explicit user request (2026-07-15)** — the user hand-edited `wolves-intro-sequence.ts` directly to choose which lines survive and rewrote several (including two new multi-line beats: "Until the Birth of AI, / and Society Decided, that / Guardians were not only unnecessary, but a threat."). The surviving cues' `start`/`end` timestamps were then retimed proportionally (each cue's original share of the total duration, scaled to fit exactly 45s) to close the dead/black gaps left by the removed lines. Multi-line cue text requires `white-space: pre-line` on `.wolves-intro-overlay-text` (added the same day) since this component intentionally never uses `v-html` — the user's line breaks are authored as real `\n` characters via a TS template literal, not `<br>` markup. **Widened from 45s to 60s the same day, per a follow-up user request**, again by proportionally rescaling every cue's `start`/`end` (same methodology, new target duration). **Widened from 60s to 75s on 2026-07-16, per the approved Task 1 user request**, again by proportionally rescaling every cue's `start`/`end` so the exact same surviving cues now span the full 75s runtime. The same request asked for a readability improvement to the Clarke-style "for the heart of any race is destroyed, and its will to survive is utterly broken" line; that quote remains authored as a single multi-line cue (preserving the user's authored line breaks) and was not split into separate sequential cues. Do not re-collapse, split, or otherwise alter that authored cue text, and do not re-expand `duration` beyond 75 or restore the originally-removed lines without a fresh user request.
