...
- **Trustee plates render in burnished silver** via the `trustee: true` cue field (`wolves-guardian-plate-trustee` CSS in `WolvesIntroOverlay.vue`): silver horizons/crest/label/burst (`#d1d5db`/`#e5e7eb`) and the `TRUSTEE // GUARDIAN` label, distinct from the default blue plate. Reserved for Universal Blue trustees — Bob Killen and Christoph Blecker use this intro treatment; Jorge Castro's Ghosts In The Mist theater plate in `WolvesComicReader.vue` mirrors the standard silver palette. Do not apply to other guardians without a fresh user request.
...
- **Christoph Blecker is a leader and trustee**, with `leader: true` and `trustee: true` rendering his **`TRUSTEE // GUARDIAN`** label and silver plate. His displayed name alone remains gold through `goldName: true` and `.wolves-guardian-plate-name-gold`, preserving the leader recognition without changing his trustee classification or other trustees' names. His window runs 83-96s, matching Natali Vlatko's end. His title line carries two segments ("First Among Equals — The North Star"), joined and rendered with equal visual weight. **"Uncompromising Purity" and "Platinum Member" (added 2026-07-15, the latter with a `blingTitle: 'Platinum Member'` shimmer) were removed the same day** per a follow-up explicit user request to drop back to just the original two titles — do not re-add either without a fresh user request. `blingTitle` must match a substring of the cue's title exactly (case-sensitive) or it silently renders with no effect.
